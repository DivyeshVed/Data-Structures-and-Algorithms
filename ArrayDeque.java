...
public class ArrayDeque<T> {
    ...
    public void addFirst(T data) {
        if (data == null) {
            // Throwing an exceptions if the data entered is null.
        ...
        if (size < backingArray.length) {
            if (front - 1 < 0) {
                ...
                int position = mod(--front, backingArray.length);
                front = position;
            }
            backingArray[front] = data;
            size++;
        } else if (size == backingArray.length) {
            // Creating a new backing array, which is double the size of the current backing array.
            T[] newBackingArray = (T[]) new Object[backingArray.length * 2];
            // Adding the data to the first index in the array. It's a good idea to start with this step.
            newBackingArray[0] = data;
            // Iterating through the old array and adding to
            for (int a = 0; a < backingArray.length; a++) {
                newBackingArray[a + 1] = backingArray[mod(front + a, backingArray.length)];
            }
            front = 0;
            size++;
    ...
    public void addLast(T data) {
        ...
        if (size == 0) {
            ...
            T[] newBackingArray = (T[]) new Object[backingArray.length * 2];
            for (int b = 0; b < backingArray.length; b++) {
                newBackingArray[b] = backingArray[mod(front + b, backingArray.length)];
            }
            newBackingArray[mod(size, backingArray.length)] = data;
            front = 0;
            size++;
            backingArray = newBackingArray;
        } else {
            int position = mod(front + size, backingArray.length);
    ...
    public T removeLast() {
        ...
        if (size == 0) {
            ...
            int position = mod(size + front - 1, backingArray.length);
            data = backingArray[position];
            backingArray[position] = null;
            size--;
        }
        return data;
    }
